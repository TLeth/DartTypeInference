...
for benchmark in $benchmarks; do
    if [ -d benchmarks/$benchmark ]; then
        ...
        if [ ! $? -eq 0 ]; then
            ...
            dart --old_gen_heap_size=1000m bin/analyze.dart --skip-sdk --skip-packages -w --json --actual-basedir inferred --expected-basedir benchmarks --benchmarkdir inferred/$benchmark --dart-sdk ${dartDir%bin/dart} inferred/$benchmark/$entryfiledir
