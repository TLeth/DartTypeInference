  ...
  void processSubclassesOfNativeClasses(Iterable<LibraryElement> libraries) {
        ...
        if (element.isClass) {
          ...
          if (extendsName != null) {
            Set<ClassElement> potentialSubclasses =
                potentialExtends.putIfAbsent(extendsName, () => new Set<ClassElement>());
    ...
    void walkPotentialSubclasses(ClassElement element) {
      ...
      if (nativeSuperclass != null) {
        ...
        if (!element.isNative) {
          nonNativeSubclasses.putIfAbsent(
              nativeSuperclass,
              () => new Set<ClassElement>()).add(element);
        }
...
class NativeResolutionEnqueuer extends NativeEnqueuerBase {
    ...
    for (String tag in nativeTagsOfClass(classElement)) {
      ...
      if (owner != null) {
        if (owner != classElement) {
          compiler.internalError(
              classElement,
              "Tag '$tag' already in use by '${owner.name}'");
        }
      } else {
  ...
  void logSummary(log(message)) {
    log(
        'Resolved ${registeredClasses.length} native elements used, '
...
class NativeCodegenEnqueuer extends NativeEnqueuerBase {
  ...
  void logSummary(log(message)) {
    log(
        'Compiled ${registeredClasses.length} native classes, '
