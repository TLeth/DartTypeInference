...
class OldEmitter implements Emitter {
  ...
  List<jsAst.Expression>
      cspPrecompiledConstructorNamesFor(OutputUnit outputUnit) {
  ...
  jsAst.FunctionDeclaration
      buildCspPrecompiledFunctionFor(OutputUnit outputUnit) {
      ...
      if (compiler.hasIncrementalSupport) {
        ClassBuilder builder =
          ...
          classEmitter.generateClass(
              classElement,
              builder,
              additionalProperties[classElement]);
          return builder;
        });
  ...
  void emitFinishClassesInvocationIfNecessary(CodeBuffer buffer) {
    if (needsDefineClass) {
      buffer.write(
          '$finishClassesName($classesCollector,' '$_$isolateProperties,' '${_}null)$N');
  ...
  void emitMakeConstantList(CodeBuffer buffer) {
    buffer.write(jsAst.prettyPrint(js.statement(r'''#.# = function(list) {
        ...
        [namer.isolateName, makeConstListProperty, initName, initName]),
        compiler,
  ...
  jsAst.Expression generateDispatchPropertyNameInitialization() {
    ...
    return js(
        '# = #("dispatch_record")',
        [dispatchPropertyNameAccess, getIsolateTagAccess]);
  }
  ...
  emitMain(CodeBuffer buffer) {
    ...
    if (backend.needToInitializeIsolateAffinityTag) {
      buffer.write(
          jsAst.prettyPrint(
              generateIsolateAffinityTagInitialization(),
              compiler,
              monitor: compiler.dumpInfoTask));
      buffer.write(N);
    }
    if (backend.needToInitializeDispatchProperty) {
      ...
      buffer.write(
          jsAst.prettyPrint(
              generateDispatchPropertyNameInitialization(),
              compiler,
              monitor: compiler.dumpInfoTask));
      buffer.write(N);
    }
...
(function (callback) {
  ...
  if (typeof dartMainRunner === "function") {
    ...
    buffer.write(';');
    buffer.write(
        jsAst.prettyPrint(invokeMain, compiler, monitor: compiler.dumpInfoTask));
    buffer.write(N);
  ...
  void emitInitFunction(CodeBuffer buffer) {
    ...
    buffer.write(
        jsAst.prettyPrint(decl, compiler, monitor: compiler.dumpInfoTask).getText());
  ...
  void writeLibraryDescriptors(CodeBuffer buffer, LibraryElement library) {
    ...
    buffer
        ...
        ..write(
            metadata == null ?
                "" :
                jsAst.prettyPrint(metadata, compiler, monitor: compiler.dumpInfoTask))
        ..write(',$_')
        ..write(namer.globalObjectFor(library))
        ..write(',$_')
        ..write(
            jsAst.prettyPrint(initializers, compiler, monitor: compiler.dumpInfoTask))
        ..write(library == compiler.mainApp ? ',${n}1' : "")
        ..write('],$n');
  }

  void emitPrecompiledConstructor(OutputUnit outputUnit, String constructorName,
      jsAst.Expression constructorAst) {
    cspPrecompiledFunctionFor(
        outputUnit).add(
            new jsAst.FunctionDeclaration(
  ...
  String deferredPartFileName(OutputUnit outputUnit, {bool addExtension:
      true}) {
    String outPath =
        compiler.outputUri != null ? compiler.outputUri.path : "out";
  ...
  void checkEverythingEmitted(Iterable<Element> elements) {
    ...
    if (!compiler.hasIncrementalSupport) {
      ...
      pendingStatics.forEach(
          (element) => compiler.reportInfo(element, MessageKind.GENERIC, {
  ...
  void emitMainOutputUnit(Map<OutputUnit, String> deferredLoadHashes,
    ...
    if (compiler.hasIncrementalSupport) {
      mainBuffer.add(
          '(this.\$dart_unsafe_eval =' ' this.\$dart_unsafe_eval || Object.create(null))'
    ...
    mainBuffer.add(
        'function dart()$_{$n' '${_}${_}this.x$_=${_}0$N' '${_}${_}delete this.x$N'
            '}$n');
    ...
    if (elementDescriptors.isNotEmpty) {
      ...
      mainBuffer
          ..write('(')
          ..write(
              jsAst.prettyPrint(getReflectionDataParser(classesCollector, backend), compiler))
          ..write(')')
          ..write('([$n')
          ..add(libraryBuffer)
          ..write('])$N');
      ...
      cspBuffer.write("""
{
  ...
  CodeBuffer buildNativesBuffer() {
    ...
    nativeEmitter.generateNativeClasses(
        nativeClasses,
        mainBuffer,
...
''';
  ...
  void emitDeferredBoilerPlate(CodeBuffer buffer, Map<OutputUnit,
      String> deferredLoadHashes) {
    // Function for checking if a hunk is loaded given its hash.
    buffer.write(
        jsAst.prettyPrint(
            js(
                '# = function(hunkHash) {' '  return !!$deferredInitializers[hunkHash];' '}',
    ...
    buffer.write(
        jsAst.prettyPrint(
            js(
                '# = function(hunkHash) {' '  $deferredInitializers[hunkHash]('
    ...
    void emitMapping(String name, Map<String, List<String>> mapping) {
      ...
      mapping.forEach((String key, List<String> values) {
        properties.add(
            new jsAst.Property(
                js.escapedString(key),
                new jsAst.ArrayInitializer.from(values.map(js.escapedString))));
      });
  ...
  Map<OutputUnit, String> emitDeferredCode(Map<OutputUnit,
      CodeBuffer> deferredBuffers) {
    ...
    for (OutputUnit outputUnit in compiler.deferredLoadTask.allOutputUnits) {
      ...
      outputBuffer
          ...
          ..write(
              '${deferredInitializers}.current$_=$_' 'function$_(${globalsHolder}) {$N');
      for (String globalObject in Namer.reservedGlobalObjectNames) {
        outputBuffer.write(
            'var $globalObject$_=$_' '${globalsHolder}.$globalObject$N');
      }
      outputBuffer
          ..write('var init$_=$_${globalsHolder}.init$N')
          ..write(
              'var ${namer.isolateName}$_=$_' '${globalsHolder}.${namer.isolateName}$N');
      ...
      outputBuffer.add(
          '${deferredInitializers}["$hash"]$_=$_' '${deferredInitializers}.current$N');
