...
import 'elements/modelx.dart' show VoidElementX, LibraryElementX,
    BaseClassElementX, TypeDeclarationElementX, TypedefElementX;
...
abstract class DartType {
  ...
  static void visitList(List<DartType> types, DartTypeVisitor visitor, var
      argument) {
    for (DartType type in types) {
...
class MalformedType extends DartType {
  ...
  MalformedType(this.element, this.userProvidedBadType, [this.typeArguments =
      null]);
...
class InterfaceType extends GenericType {
  InterfaceType(ClassElement element, [List<DartType> typeArguments = const
      <DartType>[
      ]])
...
class FunctionType extends DartType {
  ...
  factory FunctionType(FunctionTypedElement element, [DartType returnType =
      const DynamicType(), List<DartType> parameterTypes = const <DartType>[
  ...
  FunctionType.internal(FunctionTypedElement this.element, [DartType
      this.returnType = const DynamicType(), this.parameterTypes = const <DartType>[
...
class TypedefType extends GenericType {
  TypedefType(TypedefElement element, [List<DartType> typeArguments = const
      <DartType>[
      ]])
      : super(element, typeArguments);
...
class ResolvedTypedefType extends TypedefType {
  ...
  ResolvedTypedefType(TypedefElement element, List<DartType> typeArguments,
      this.alias)
      : super(element, typeArguments) {
    assert(invariant(
        element,
  ...
  static int compare(DartType a, DartType b) {
      ...
      if (isDefinedByDeclaration(b)) {
        ...
        if (a.isTypeVariable) {
          ...
          if (b.isTypeVariable) {
            ...
            return compareList(
                (a as GenericType).typeArguments,
    ...
    if (a.isFunctionType) {
      if (b.isFunctionType) {
        ...
        if (result != 0) return result;
        result =
            compareList(aFunc.optionalParameterTypes, bFunc.optionalParameterTypes);
        if (result != 0) return result;
  ...
  DartType computeLeastUpperBoundFunctionTypes(FunctionType a, FunctionType b) {
    ...
    while (aIndex < aNamedParameters.length &&
      ...
      if (result == 0) {
        ...
        namedParameterTypes.add(
            computeLeastUpperBound(
                aNamedParameterTypes[aIndex],
                bNamedParameterTypes[bIndex]));
      }
      if (result <= 0) {
        aIndex++;
      }
  ...
  InterfaceType computeMoreSpecific(ClassElement element,
    ...
    if (supertypeInstance.accept(this, supertype)) {
      ...
      List<DartType> typeArguments = new List<DartType>.generate(
          variables.length,
          (int index) => constraintMap[variables[index]]);
