...
class IntValue extends Value {
  ...
  Value operator +(other) {
    ...
    var constant = constantSystem.add.fold(
        ...
        constantSystem.createInt(other.value));
...
class Range {
  ...
  Range intersection(Range other) {
    ...
    if (low == const UnknownValue()) {
      if (lower is IntValue) low =
          lower; else if (other.lower is IntValue) low = other.lower; else low = lower;
    }
    if (up == const UnknownValue()) {
      if (upper is IntValue) up =
          upper; else if (other.upper is IntValue) up = other.upper; else up = upper;
    }
...
class SsaValueRangeAnalyzer extends HBaseVisitor implements OptimizationPhase {
  ...
  Range visitBoundsCheck(HBoundsCheck check) {
    ...
    Range newIndexRange =
        indexRange.intersection(info.newNormalizedRange(info.intZero, maxIndex));
  ...
  Range visitInvokeDynamicMethod(HInvokeDynamicMethod invoke) {
    if ((invoke.inputs.length == 3) &&
        (invoke.selector.name == "%")) return handleInvokeModulo(invoke);
  ...
  Range handleBinaryOperation(HBinaryArithmetic instruction) {
    ...
    return instruction.operation(
        constantSystem).apply(ranges[instruction.left], ranges[instruction.right]);
  }
  ...
  Range computeConstrainedRange(BinaryOperation operation, Range leftRange,
      Range rightRange) {
    Range range;
    if (operation == const LessOperation()) {
      range =
          info.newNormalizedRange(const MinIntValue(), rightRange.upper - info.intOne);
    } else if (operation == const LessEqualOperation()) {
      range = info.newNormalizedRange(const MinIntValue(), rightRange.upper);
    } else if (operation == const GreaterOperation()) {
      range =
          info.newNormalizedRange(rightRange.lower + info.intOne, const MaxIntValue());
    } else if (operation == const GreaterEqualOperation()) {
