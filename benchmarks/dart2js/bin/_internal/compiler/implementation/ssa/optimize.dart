  ...
  void optimize(CodegenWorkItem work, HGraph graph) {
    ...
    measure(() {
      ...
      List<OptimizationPhase> phases = <OptimizationPhase>[
        ...
        new SsaInstructionSimplifier(constantSystem, backend, work), new SsaCheckInserter(backend, work, context.boundsChecked), new SsaSimplifyInterceptors(compiler, constantSystem, work), dce =
            new SsaDeadCodeEliminator(compiler), new SsaTypePropagator(compiler)];
...
class SsaInstructionSimplifier extends HBaseVisitor implements OptimizationPhase
  ...
  HInstruction visitInvokeDynamicMethod(HInvokeDynamicMethod node) {
    ...
    if (element !=
        null // If we found out that the only target is a [:noSuchMethod:],
    // we just ignore it.
    &&
        element.isFunction
  ...
  HInstruction foldBinary(BinaryOperation operation, HInstruction left,
      HInstruction right) {
  ...
  void simplifyCondition(HBasicBlock block, HInstruction condition,
      bool value) {
  ...
  HInstruction visitIf(HIf node) {
    ...
    if (isNegated) {
      ...
      Iterable<HInstruction> dominating =
          condition.usedBy.where((user) => user is HNot && user.dominates(node));
  ...
  HInstruction visitIs(HIs node) {
    ...
    if (expression.isInteger(compiler)) {
      ...
      if (expressionMask.union(typeMask, classWorld) == typeMask) {
        return graph.addConstantBool(true, compiler);
      } else if (expressionMask.intersection(
          typeMask,
  ...
  HInstruction visitStringify(HStringify node) {
    ...
    if (input.isConstant()) {
      ...
      return graph.addConstant(
          constantSystem.createString(primitive.toDartString()),
          compiler);
    }
    return node;
  }
...
class SsaCheckInserter extends HBaseVisitor implements OptimizationPhase {
  ...
  HBoundsCheck insertBoundsCheck(HInstruction indexNode, HInstruction array,
      HInstruction indexArgument) {
    Compiler compiler = backend.compiler;
    HFieldGet length = new HFieldGet(
        backend.jsIndexableLength,
        array,
        backend.positiveIntType,
  ...
  void visitInvokeDynamicMethod(HInvokeDynamicMethod node) {
    ...
    insertBoundsCheck(
        node,
        node.receiver,
        graph.addConstantInt(0, backend.compiler));
  }
}
...
class SsaDeadPhiEliminator implements OptimizationPhase {
  ...
  void visitGraph(HGraph graph) {
    ...
    for (int i = blocks.length - 1; i >= 0; i--) {
      ...
      while (current != null) {
        next = current.next;
        if (!livePhis.contains(
            current) // TODO(ahe): Not sure the following is correct.
...
class SsaGlobalValueNumberer implements OptimizationPhase {
  ...
  void moveLoopInvariantCodeFromBlock(HBasicBlock block, HBasicBlock loopHeader,
    ...
    bool firstInstructionInLoop =
        block == loopHeader // Compensate for lack of code motion.
    ||
...
class SsaTypeConversionInserter extends HBaseVisitor implements
    OptimizationPhase {
  ...
  void insertTypePropagationForDominatedUsers(HBasicBlock dominator,
      HInstruction input, TypeMask convertedType) {
  ...
  void visitBasicBlock(HBasicBlock block) {
    if (block.predecessors.length == 0) {
      ...
      for (int i = 1; i < block.predecessors.length; i++) {
        memorySet =
            memorySet.intersectionFor(memories[block.predecessors[i].id], block, i);
      }
    }
  ...
  void visitForeignNew(HForeignNew instruction) {
    ...
    instruction.element.forEachInstanceField((_, Element member) {
      memorySet.registerFieldValue(
          member,
          instruction,
          instruction.inputs[argumentIndex++]);
...
class MemorySet {
   ...
   */
  final Map<Element, Map<HInstruction, HInstruction>> fieldValues = <Element,
  ...
  bool mayAlias(HInstruction first, HInstruction second) {
    ...
    TypeMask intersection =
        first.instructionType.intersection(second.instructionType, compiler.world);
   ...
   */
  void registerFieldValueUpdate(Element element, HInstruction receiver,
      HInstruction value) {
   ...
   */
  void registerFieldValue(Element element, HInstruction receiver,
      HInstruction value) {
    if (element.isNative) return; // TODO(14955): Remove this restriction?
    Map<HInstruction, HInstruction> map =
        fieldValues.putIfAbsent(element, () => <HInstruction, HInstruction>{});
   ...
   */
  void registerKeyedValue(HInstruction receiver, HInstruction index,
      HInstruction value) {
    Map<HInstruction, HInstruction> map =
   ...
   * places that may be affected by this update.
   */
  void registerKeyedValueUpdate(HInstruction receiver, HInstruction index,
      HInstruction value) {
    nonEscapingReceivers.remove(value);
   ...
   */
  MemorySet intersectionFor(MemorySet other, HBasicBlock block,
    ...
    fieldValues.forEach((element, values) {
      ...
      values.forEach((receiver, value) {
        HInstruction instruction =
            findCommonInstruction(value, otherValues[receiver], block, predecessorIndex);
        if (instruction != null) {
    ...
    keyedValues.forEach((receiver, values) {
      ...
      values.forEach((index, value) {
        HInstruction instruction =
            findCommonInstruction(value, otherValues[index], block, predecessorIndex);
        if (instruction != null) {
