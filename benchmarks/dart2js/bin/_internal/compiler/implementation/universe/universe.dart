...
class Universe {
  ...
  bool hasMatchingSelector(Set<Selector> selectors, Element member,
      World world) {
...
class Selector {
  ...
  Selector.internal(this.kind, this.name, this.library, this.argumentCount,
      this.namedArguments, this._orderedNamedArguments, this.hashCode) {
  ...
  factory Selector(SelectorKind kind, String name, LibraryElement library,
      int argumentCount, [List<String> namedArguments]) {
    if (!isPrivateName(name)) library = null;
    if (namedArguments == null) namedArguments = const <String>[];
    int hashCode =
        computeHashCode(kind, name, library, argumentCount, namedArguments);
    List<Selector> list =
    ...
    List<String> orderedNamedArguments =
        namedArguments.isEmpty ? const <String>[] : <String>[];
  ...
  factory Selector.fromElement(Element element) {
    ...
    if (element.isFunction) {
      ...
      throw new SpannableAssertionFailure(
          element,
          "Can't get selector from $element");
    }
  }
  ...
  factory Selector.callClosure(int arity, [List<String> namedArguments]) =>
      new Selector(SelectorKind.CALL, CALL_NAME, null, arity, namedArguments);
  ...
  bool get isIndexSet =>
      identical(kind, SelectorKind.INDEX) && argumentCount == 2;
   ...
   */
  /*<T>*/ bool addArgumentsToList(Link<Node> arguments, List /*<T>*/ list,
      FunctionElement element, /*T*/ compileArgument(Node argument), /*T*/
  compileDefaultValue(ParameterElement element), World world) {
   ...
   */
  static bool addForwardingElementArgumentsToList(FunctionElement caller,
      List list, FunctionElement callee, compileArgument(Element element),
      compileConstant(Element element), World world) {
  ...
  bool match(SelectorKind kind, String name, LibraryElement library,
      int argumentCount, List<String> namedArguments) {
  ...
  static int computeHashCode(SelectorKind kind, String name,
      LibraryElement library, int argumentCount, List<String> namedArguments) {
...
class TypedSelector extends Selector {
  ...
  factory TypedSelector(TypeMask mask, Selector selector, World world) {
    ...
    Map<TypeMask, TypedSelector> map = canonicalizedValues.putIfAbsent(
        untyped,
  ...
  factory TypedSelector.exact(ClassElement base, Selector selector, World world)
      =>
      new TypedSelector(new TypeMask.exact(base, world), selector, world);

  factory TypedSelector.subclass(ClassElement base, Selector selector,
      World world) =>
      new TypedSelector(new TypeMask.subclass(base, world), selector, world);

  factory TypedSelector.subtype(ClassElement base, Selector selector,
      World world) =>
      new TypedSelector(new TypeMask.subtype(base, world), selector, world);
  ...
  Selector extendIfReachesAll(Compiler compiler) {
    ...
    return canReachAll ?
        new TypedSelector(compiler.typesTask.dynamicType, this, compiler.world) :
        this;
  }
}
