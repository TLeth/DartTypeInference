...
import '../elements/elements.dart' show Element, Name, PublicName, Member,
    MemberSignature, LibraryElement, ClassElement, MixinApplicationElement;
...
abstract class MembersCreator {
  ...
  MembersCreator(Compiler this.compiler, ClassElement this.cls, Iterable<String>
      this.computedMemberNames, Map<Name, Member> this.classMembers) {
    assert(invariant(
        cls,
        cls.isDeclaration,
  ...
  Map<Name, Member> computeClassMembers(String nameText, Setlet<Name> names) {
    ...
    if (cls.isMixinApplication) {
      ...
      void createMember(Element element) {
        ...
        void addDeclaredMember(Name name, DartType type,
        ...
        if (element.isField) {
          ...
          if (!element.isConst && !element.isFinal) {
            addDeclaredMember(
                name.setter,
                type,
                new FunctionType.synthesized(const VoidType(), <DartType>[type]));
          }
  ...
  void checkInterfaceMember(Name name, MemberSignature interfaceMember,
      Member classMember) {
    if (classMember != null) {
    ...
    if (interfaceMember is DeclaredMember &&
      ...
      reportWarning(MessageKind singleKind, MessageKind multipleKind,
          MessageKind explicitlyDeclaredKind, [MessageKind implicitlyDeclaredKind]) {
        Member inherited = interfaceMember.declarations.first;
        reportMessage(interfaceMember, MessageKind.UNIMPLEMENTED_METHOD, () {
          compiler.reportWarning(
              cls,
              interfaceMember.declarations.length == 1 ? singleKind : multipleKind,
              {
            'class': cls.name,
            'name': name.text,
          ...
          for (Member inherited in interfaceMember.declarations) {
            compiler.reportInfo(
                inherited.element,
                inherited.isDeclaredByField ? implicitlyDeclaredKind : explicitlyDeclaredKind,
                {
  ...
  void reportErrorWithContext(Element errorneousElement,
      MessageKind errorMessage, Element contextElement, MessageKind contextMessage) {
  ...
  static void computeClassMembersByName(Compiler compiler, ClassMemberMixin cls,
      String name) {
...
class InterfaceMembersCreator extends MembersCreator {
  ...
  InterfaceMembersCreator(Compiler compiler, ClassElement cls,
      Iterable<String> computedMemberNames, Map<Name, Member> classMembers, Map<Name,
      MemberSignature> this.interfaceMembers)
      : super(compiler, cls, computedMemberNames, classMembers);
  ...
  Map<Name, Setlet<Member>> computeSuperInterfaceMembers(String name,
    ...
    void inheritInterfaceMember(InterfaceType supertype, MemberSignature member)
      ...
      Setlet<Member> members =
          inheritedInterfaceMembers.putIfAbsent(member.name, () => new Setlet<Member>());
    ...
    for (Link<DartType> link =
        cls.interfaces; !link.isEmpty; link = link.tail) {
      InterfaceType superinterface = link.head;
      if (names != null) {
        MembersCreator._computeClassMember(
            compiler,
            superinterface.element,
            name,
            names);
        for (Name memberName in names) {
          inheritInterfaceMember(
  ...
  void computeInterfaceMembers(Map<Name,
      Setlet<Member>> inheritedInterfaceMembers, Map<Name, Member> declaredMembers) {
    InterfaceType thisType = cls.thisType;
      ...
      if (declared != null) {
        ...
        outer: for (Member inherited in inheritedMembers) {
          ...
          for (MemberSignature other in inheritedMembers) {
            if (!compiler.types.isSubtype(
                inherited.functionType,
                other.functionType)) {
              continue outer;
            }
          }
          subtypesOfAllInherited.putIfAbsent(
              inherited.functionType,
        ...
        if (someAreGetters && !allAreGetters) {
          ...
          for (Member inherited in inheritedMembers) {
            ...
            if (inherited.isMethod) {
              ...
              assert(invariant(
                  cls,
                  inherited.isGetter,
                  message: 'Conflicting member is neither a method nor a ' 'getter.'));
              if (inherited.isDeclaredByField) {
          ...
          if (members.length != 1) {
            ...
            inherited =
                new SyntheticMember(members, inherited.type, inherited.functionType);
          }
...
abstract class ClassMemberMixin implements ClassElement {
  ...
  MembersCreator _prepareCreator(Compiler compiler) {
    ...
    return interfaceMembersAreClassMembers ?
        new ClassMembersCreator(compiler, this, computedMemberNames, classMembers) :
        new InterfaceMembersCreator(
            compiler,
            this,
            computedMemberNames,
            classMembers,
